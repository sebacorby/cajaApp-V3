# APPCAJA-V3-REMEDIATION-VALIDATION-v1.0.4


## 1. Objetivo único


Remediar únicamente los residuos locales enumerados en esta instrucción y, sin abrir ningún cambio funcional adicional, validar desde cero el estado consolidado de CajaApp V3 después de la implementación completa de P1, P2, P3 y P4.
...
La tarea tiene dos fases indivisibles: una remediación local estrictamente acotada y una validación consolidada completa. Fuera de las operaciones expresamente autorizadas en 5A, el agente no implementa, adapta ni corrige el producto.
...
- Carpeta única de entrega: `I:\cajaApp-V3\architecture-handoff\agents-to-architect\pending-validation\APPCAJA-V3-REMEDIATION-VALIDATION-evidence-v1.0.4`.
...
- `APPCAJA-V3-REMEDIATION-VALIDATION-v1.0.3` terminó en `FAIL`: BOM en `schema.prisma`, residuos locales `(1)` y `-Stop` todavía estricto frente a ocupantes externos del puerto 3000;
- el arquitecto ya publicó en Drive el schema sin BOM, eliminó los duplicados visibles, limpió `dist/.next` y reparó `-Stop` para ignorar procesos externos sin finalizarlos;
- la evidencia `v1.0.3` debe archivarse en `rejected` y no puede reutilizarse;
...
- la campaña `v1.0.2` quedó invalidada porque detectó un defecto real en `cajaapp-headless-up.ps1`; el arquitecto reparó el script canónico conservando su ruta e ID.
- no reutilizar hashes, backup de campaña, logs ni evidencia de `v1.0.2`.
...
## 5A. Remediación local autorizada antes de validar


Esta fase existe porque la campaña `v1.0.3` confirmó residuos locales de sincronización y un BOM real. Ejecutarla una sola vez antes de obtener los hashes iniciales de la campaña.


### 5A.1 Archivar la evidencia anterior
...
```text
I:\cajaApp-V3\architecture-handoff\agents-to-architect\pending-validation\APPCAJA-V3-REMEDIATION-VALIDATION-evidence-v1.0.3
```


moverla completa, sin editar su contenido, a:


```text
I:\cajaApp-V3\architecture-handoff\agents-to-architect\rejected\APPCAJA-V3-REMEDIATION-VALIDATION-evidence-v1.0.3-FAIL
```


### 5A.2 Normalizar `schema.prisma`


Comprobar los primeros bytes de:


```text
I:\cajaApp-V3\workspace\backend\prisma\schema.prisma
```


El archivo debe comenzar con los bytes ASCII de `generator`: `67 65 6E 65` y no con `EF BB BF`.


Si todavía conserva BOM por retraso de sincronización, quitar exclusivamente esos tres bytes y reescribir el mismo contenido como UTF-8 sin BOM. No reformatear, no agregar modelos y no cambiar tokens del schema. Registrar hash antes y después en `00-remediation.md`.


### 5A.3 Eliminar únicamente residuos inequívocos


Eliminar sólo si existen los siguientes archivos locales:


```text
workspace\backend\src\modules\ai\TEMP-ai-extraction (1).service.ts
workspace\backend\src\modules\ai\TEMP-ai-extraction.service.ts
workspace\backend\src\modules\global-search\global-search (1).controller.ts
workspace\backend\src\modules\global-search\global-search (1).routes.ts
workspace\backend\src\modules\global-search\global-search (1).schemas.ts
workspace\backend\src\modules\global-search\global-search (1).service.ts
workspace\backend\src\modules\movements\categories (1).service.ts
workspace\backend\tests\movements\categories (1).rules.test.ts
workspace\backend\tests\global-search\global-search (1).service.test.ts
...
workspace\frontend\tests\categories (1).spec.ts
```


También eliminar por completo, si existen, los directorios generados:


```text
workspace\backend\dist
workspace\frontend\.next
workspace\frontend\test-results
workspace\frontend\playwright-report
```


No eliminar archivos canónicos sin sufijo. No usar patrones amplios fuera de esta lista. No tocar `dev.db`, `dev.db.clean-backup`, migraciones canónicas ni `node_modules` en esta fase.


### 5A.4 Verificación posterior obligatoria


Confirmar y registrar:


- `schema.prisma` sin BOM;
- ausencia de todos los residuos enumerados;
- presencia de cada archivo canónico equivalente;
- `cajaapp-headless-up.ps1 -Stop -JsonOnly` devuelve `ok:true` aunque el puerto 3000 pertenezca a Docker/WSL;
- el comando anterior no finaliza procesos externos;
- no quedan procesos CajaApp ni puertos de campaña ocupados.


Si una operación distinta de las enumeradas fuera necesaria, detener y declarar `BLOCKED`. Tras completar 5A, el resto de la campaña vuelve a ser exclusivamente de validación.
...
- modificar código fuente, tests, migraciones, schemas, prompts, configuración, dependencias o lockfiles, salvo quitar el BOM exactamente como autoriza 5A.2;
- eliminar, renombrar, mover o corregir archivos, salvo los residuos exactos y la evidencia anterior autorizados en 5A;
- crear wrappers, runners o scripts auxiliares `.ps1`, `.bat`, `.cmd`, `.sh`, `.py`, `.js`, `.mjs` o `.ts`;
- sustituir PowerShell por Bash o viceversa;
- iniciar backend o frontend manualmente, con `npm`, `node`, `Start-Process`, `cmd.exe` o cualquier comando directo;
- usar scripts alternativos a `cajaapp-headless-up.ps1`;
- liberar puertos matando procesos externos a CajaApp;
...
Después de completar 5A, ante cualquier defecto nuevo registrar evidencia y continuar cuando sea seguro. No corregirlo.
...
```text
I:\cajaApp-V3\architecture-handoff\agents-to-architect\pending-validation\APPCAJA-V3-REMEDIATION-VALIDATION-evidence-v1.0.4
```
...
- `-Stop -JsonOnly` ignora procesos externos en los puertos por defecto y no los finaliza;
- el arranque sigue siendo estricto: si un puerto solicitado está ocupado por un proceso externo, falla y exige elegir otro puerto;
- no contiene `taskkill /IM node.exe /F` ni una operación equivalente global.
...
7. Crear una copia nueva de campaña dentro de la carpeta de evidencia o en `%TEMP%\cajaapp-sqlite-backups`, con nombre inequívoco `PRE-v1.0.4`.
...
### 11.0 Selección segura de puertos


El puerto frontend 3000 estaba ocupado por procesos externos en la campaña anterior. No se debe matar ni reiniciar Docker, WSL, `com.docker.backend`, `wslrelay` ni ningún proceso ajeno a CajaApp.


Antes del arranque, elegir dos puertos TCP libres mediante `Get-NetTCPConnection` y registrar la evidencia. Preferencia inicial:
...
Si alguno está ocupado, elegir otro puerto libre por encima de 11436. Los puertos seleccionados deben mantenerse constantes durante smoke y Playwright.


El ecosistema se inicia exclusivamente así:
...
& "I:\cajaApp-V3\cajaapp-headless-up.ps1" `
  -Rebuild `
  -JsonOnly `
  -BackendPort <BACKEND_PORT_LIBRE> `
  -FrontendPort <FRONTEND_PORT_LIBRE>
```


No se permite reemplazar este comando por arranque manual. Si el script falla, registrar stdout, stderr, JSON, logs y `state.json`; clasificar el defecto y no improvisar otro mecanismo de arranque.
...
```powershell
& "I:\cajaApp-V3\cajaapp-headless-up.ps1" -Rebuild -JsonOnly -BackendPort <BACKEND_PORT_LIBRE> -FrontendPort <FRONTEND_PORT_LIBRE>
```
...
- no debe quedar ningún registro `UAT-v1.0.4-`;
...
4. Restaurar la copia limpia `PRE-v1.0.4` sobre `prisma\dev.db`.
...
- campaña `APPCAJA-V3-REMEDIATION-VALIDATION-v1.0.4`;
- root exacto;
...
2. Confirmar que toda la evidencia está dentro de la carpeta `v1.0.4`.
...
Si la evidencia `v1.0.4` es completa y el arquitecto acepta el `PASS`, quedará habilitada la emisión de:
