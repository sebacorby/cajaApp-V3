...
if (before.packageJson === EXPECTED_AFTER.packageJson && before.packageLock === EXPECTED_AFTER.packageLock) {
  console.log(JSON.stringify({ vertical: "APP-SEC-DEPS-001", version: VERSION, alreadyMaterialized: true, hashes: before }, null, 2));
  process.exit(0);
}

assert(before.packageJson === EXPECTED_BEFORE.packageJson, `package.json fuera de baseline: ${before.packageJson}`);
assert(before.packageLock === EXPECTED_BEFORE.packageLock, `package-lock.json fuera de baseline: ${before.packageLock}`);
...
assert(after.packageJson === EXPECTED_AFTER.packageJson, `Hash candidato package.json inesperado: ${after.packageJson}`);
assert(after.packageLock === EXPECTED_AFTER.packageLock, `Hash candidato package-lock.json inesperado: ${after.packageLock}`);
