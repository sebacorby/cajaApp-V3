...
test.describe("Movimientos manuales", () => {
  test("crea, edita y anula un movimiento desde la UI", async ({ page }) => {
    ...
    const row = page.locator('[data-testid^="movement-row-"]:visible').filter({ hasText: description }).first();
    ...
    await page.getByTestId("save-manual-movement").click();
    await expect(
      page.locator('[data-testid^="movement-row-"]:visible').filter({ hasText: editedDescription }).first(),
    ).toBeVisible();

    page.once("dialog", (dialog) => dialog.accept());
    const editedRow = page.locator('[data-testid^="movement-row-"]:visible').filter({ hasText: editedDescription }).first();
    await editedRow.getByRole("button", { name: new RegExp(`Eliminar ${editedDescription}`) }).click();
    await expect(
      page.locator('[data-testid^="movement-row-"]:visible').filter({ hasText: editedDescription }),
    ).toHaveCount(0);
  });
});
