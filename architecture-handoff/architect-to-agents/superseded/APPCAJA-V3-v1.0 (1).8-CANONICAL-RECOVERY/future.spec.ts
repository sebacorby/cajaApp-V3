...
test.describe("Deuda y compromisos futuros", () => {
  test("muestra un compromiso confirmado y permite abrir su origen", async ({
    ...
    try {
      ...
      const monthPanel = page.getByTestId(`future-month-${monthKey}`);
      ...
      await expect(monthPanel.getByText("Confirmado", { exact: true })).toBeVisible();
      await expect(monthPanel.getByText(/12\.345,67/)).toBeVisible();
