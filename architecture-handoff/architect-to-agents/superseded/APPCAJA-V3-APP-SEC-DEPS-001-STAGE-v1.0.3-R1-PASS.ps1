...
# FIX R1: los tests resuelven fixtures desde el root versionado de staging.
$visaTarget = Join-Path $stagingRoot 'docs\08-artifacts\visa-galicia-julio2026.pdf'
$salaryTarget = Join-Path $stagingRoot 'contracts\examples\salary-receipts\salary-receipt.sanitized.base.pdf'

# Paths incorrectos utilizados por la primera ejecución. Se limpian para impedir falsos positivos.
$legacyVisaTarget = Join-Path $env:LOCALAPPDATA 'CajaApp\validation\docs\08-artifacts\visa-galicia-julio2026.pdf'
$legacySalaryTarget = Join-Path $env:LOCALAPPDATA 'CajaApp\validation\contracts\examples\salary-receipts\salary-receipt.sanitized.base.pdf'
...
function Copy-Frontend([string]$From, [string]$To) {
  ...
  $code = $LASTEXITCODE
  if ($code -gt 7) {
    throw "Robocopy falló con código $code para $To"
  }
  return $code
}

if (-not (Test-Path -LiteralPath $source -PathType Container)) {
  throw "Frontend canónico ausente: $source"
}
...
if ((Hash $sourcePackage) -ne $expectedBefore.PackageJson) {
  throw 'package.json canónico fuera del baseline.'
}
if ((Hash $sourceLock) -ne $expectedBefore.PackageLock) {
  throw 'package-lock.json canónico fuera del baseline.'
}

Remove-Item -LiteralPath $legacyVisaTarget -Force -ErrorAction SilentlyContinue
Remove-Item -LiteralPath $legacySalaryTarget -Force -ErrorAction SilentlyContinue
...
foreach ($path in @(
  ...
  if ((Hash $path) -ne $expectedBefore.PackageJson) {
    throw "package.json staging no coincide con baseline: $path"
  }
}
foreach ($path in @(
  ...
  if ((Hash $path) -ne $expectedBefore.PackageLock) {
    throw "package-lock staging no coincide con baseline: $path"
  }
}

& $node $materializer $candidate
if ($LASTEXITCODE -ne 0) {
  throw "Materializador v1.0.3 falló con exit code $LASTEXITCODE"
}
if ((Hash (Join-Path $candidate 'package.json')) -ne $expectedAfter.PackageJson) {
  throw 'Hash candidato package.json inesperado.'
}
if ((Hash (Join-Path $candidate 'package-lock.json')) -ne $expectedAfter.PackageLock) {
  throw 'Hash candidato package-lock.json inesperado.'
}
...
Assert-Leaf $visaTarget 'Fixture Visa en staging versionado'
Assert-Leaf $salaryTarget 'Fixture salario en staging versionado'
if ((Hash $visaTarget) -ne (Hash $visaSource)) {
  throw 'Hash fixture Visa no coincide con el origen canónico.'
}
if ((Hash $salaryTarget) -ne (Hash $salarySource)) {
  throw 'Hash fixture salario no coincide con el origen canónico.'
}
if (Test-Path -LiteralPath $legacyVisaTarget -PathType Leaf) {
  throw "Fixture Visa quedó en path legacy incorrecto: $legacyVisaTarget"
}
if (Test-Path -LiteralPath $legacySalaryTarget -PathType Leaf) {
  throw "Fixture salario quedó en path legacy incorrecto: $legacySalaryTarget"
}

$fixtures = @(
  [ordered]@{
    name = 'visa-galicia-julio2026.pdf'
    source = $visaSource
    target = $visaTarget
    size = (Get-Item -LiteralPath $visaTarget).Length
    sha256 = Hash $visaTarget
  },
  [ordered]@{
    name = 'salary-receipt.sanitized.base.pdf'
    source = $salarySource
    target = $salaryTarget
    size = (Get-Item -LiteralPath $salaryTarget).Length
    sha256 = Hash $salaryTarget
  }
)
