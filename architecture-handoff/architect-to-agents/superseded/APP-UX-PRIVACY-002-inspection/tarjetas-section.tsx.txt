...
type RowEdit = Partial<Pick<CardStatementRow, EditableRowField>>;
...
const EMPTY_MANUAL_PURCHASE: ManualPurchaseForm = {
  cardLast4: "",
  holderName: "",
...
function formatMoney(value: string | null, currency: "ARS" | "USD"): string {
...
function isPositiveExchangeRate(value: string): boolean {
...
function exchangeRateSourceLabel(source: string | null): string {
...
function acceptedStatementToPreview(
...
function getProjectionRange(): { from: string; to: string } {
...
function groupDescriptor(group: CardStatementGroup): string {
  const details = [
...
function rowTone(row: CardStatementRow): string {
  switch (row.rowType) {
...
function PersistedDataLoadingState() {
  return (
      ...
      <CardContent>
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-xl border bg-muted/15 p-4">
            ...
            <p className="mt-2 font-semibold tabular-nums">
              {formatMoney(totalDollars, "USD")}
            </p>
          </div>
          <div className="rounded-xl border bg-muted/15 p-4">
            <p className="text-xs text-muted-foreground">Equivalente USD en ARS</p>
            <p className="mt-2 font-semibold tabular-nums">
              {formatMoney(equivalents.usdEquivalentPesos, "ARS")}
            </p>
          </div>
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
            ...
            <p className="mt-2 font-semibold tabular-nums text-primary">
              {formatMoney(equivalents.combinedTotalPesos, "ARS")}
            </p>
          </div>
        </div>
      </CardContent>
        ...
        <CardContent className="p-6">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <MetricBox
              ...
              value={formatMoney(preview.summary.totalPesos, "ARS")}
              testId="card-statement-total-pesos"
            />
            <MetricBox
              icon={CreditCard}
              label="Total en dólares"
              value={formatMoney(preview.summary.totalDollars, "USD")}
              testId="card-statement-total-dollars"
            />
            <MetricBox
              icon={ShieldCheck}
              label="Pago mínimo"
              value={formatMoney(preview.summary.minimumPaymentPesos, "ARS")}
            />
            <MetricBox
              icon={CalendarDays}
              label="Vencimiento"
      ...
      <td className={`px-3 py-2.5 align-middle ${className ?? ""}`}>
        <div className="flex items-center gap-2">
          <span className="min-w-0 flex-1 whitespace-pre-wrap">
            {currency ? formatMoney(value, currency) : cleanText(value)}
          </span>
          {warningCount > 0 ? (
            <span title={row.warnings.join(" · ")} className="shrink-0 text-amber-600">
...
function StatementHistoryPanel({
  history,
  selectedStatementId,
  activeStatementId,
  loading,
                  ...
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    ...
                    <div className="flex flex-col gap-3 xl:items-end">
                      <div className="text-left xl:text-right">
                        <p className="font-semibold tabular-nums">{formatMoney(item.totalPesos, "ARS")}</p>
                        <p className="text-sm tabular-nums text-muted-foreground">{formatMoney(item.totalDollars, "USD")}</p>
                      </div>
                          ...
                          {itemTraceability.versions.map((version) => (
                            <div key={version.id} className="rounded-lg border p-3 text-xs">
                              ...
                              {version.isActiveForPeriod ? (
                                <p className="mt-2 font-medium text-emerald-600">Vigente para el período</p>
                              ) : null}
                            </div>
                          ))}
                        </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {updatedValues.map((item) => (
                <div key={item.monthKey} className="rounded-xl border p-4">
                  ...
                  <p className="mt-2 font-semibold tabular-nums">
                    {formatMoney(item.totalPesos, "ARS")}
                  </p>
                  <p className="mt-1 text-sm tabular-nums text-muted-foreground">
                    {formatMoney(item.totalDollars, "USD")}
                  </p>
