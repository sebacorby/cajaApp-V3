param(
  ...
  [string]$Evidence = 'C:\Users\javie\Javier.s.corbella Dropbox\Javier Corbella\cajaApp-V3\architecture-handoff\agents-to-architect\pending-validation\APPCAJA-V3-APP-SEC-DEPS-001-evidence-v1.0.1'
)
...
$staging = Join-Path $env:LOCALAPPDATA 'CajaApp\validation\APP-SEC-DEPS-001-v1.0.1\frontend'
...
if ($gates.vertical -ne 'APP-SEC-DEPS-001' -or $gates.version -ne 'v1.0.1') { throw 'Manifiesto PASS no corresponde a APP-SEC-DEPS-001 v1.0.1.' }
if ($gates.auditTotal -ne 0) { throw 'npm audit no está en cero.' }
foreach ($field in @('npmCi','npmLs','typecheck','lint','build','backendHealth','frontendHealth','playwright','cleanup','sqliteRestored')) {
  if (-not [bool]$gates.$field) { throw "Gate no confirmado: $field" }
}
if ([int]$gates.playwrightFailed -ne 0 -or [int]$gates.playwrightSkipped -ne 0 -or [int]$gates.playwrightRetries -ne 0) {
  throw 'Playwright contiene fallos, skips o retries.'
}
