...
def main() -> int:
    ...
    if not node:
        print(
            f"ERROR: node was not found in PATH. CajaApp V3 requires Node.js {REQUIRED_NODE_VERSION}.",
            file=sys.stderr,
        )
        return 1
    ...
    if version != REQUIRED_NODE_VERSION:
        print(
            f"ERROR: unsupported Node.js version {version}. CajaApp V3 requires Node.js {REQUIRED_NODE_VERSION}.",
            file=sys.stderr,
        )
        return 1
