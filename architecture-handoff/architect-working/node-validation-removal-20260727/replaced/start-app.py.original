...
def node_executable() -> str:
    ...
    if not node:
        raise RuntimeError(
            f"Node.js was not found in PATH. CajaApp V3 requires Node.js {REQUIRED_NODE_VERSION}."
        )
    version = subprocess.check_output([node, "--version"], text=True).strip()
    if version != REQUIRED_NODE_VERSION:
        raise RuntimeError(
            f"Unsupported Node.js version {version}. CajaApp V3 requires Node.js {REQUIRED_NODE_VERSION}."
        )
    log("OK", f"Node.js {version} detected.")
    return node
...
def child_popen_kwargs() -> dict[str, object]:
    if sys.platform == "win32":
        # A new process group allows controlled signal handling without orphaning
        # the child. DETACHED_PROCESS is intentionally forbidden here.
