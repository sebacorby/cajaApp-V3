...
export type FutureDebtOriginType =
  | "card_statement"
...
export interface FutureDebtRow {
  id: string;
  /** Stable identity shared by every projected installment of one purchase. */
  sourceId?: string;
  /** Original purchase date, not the projected month. */
  dateIso?: string | null;
  monthKey: string;
  description: string;
...
const ZERO_MONEY: FutureDebtMoney = { ars: "0.00", usd: "0.00" };
...
function normalizeViewSummary(response: FutureDebtResponse): FutureDebtResponse {
  const nextMonthKey = monthKeyWithOffset(response.range.from, 1);
  const nextMonth = response.months.find((month) => month.monthKey === nextMonthKey);
  return {
    ...response,
...
export async function fetchFutureDebt(
  ...
  const params = buildParams({
    ...query,
    // The matrix always needs the current statement period as its first column.
    // Some legacy callers still send false explicitly, so force the view contract here.
  ...
  const validated = assertResponse((await response.json()) as FutureDebtResponse);
  return normalizeViewSummary(validated);
}
