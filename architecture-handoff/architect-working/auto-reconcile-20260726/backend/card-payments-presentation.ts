...
function toCents(value: string | null | undefined): bigint {
  const raw = (value ?? "0").trim().replace(/\s/g, "");
  if (!raw) return 0n;
...
function chooseAdjustmentCard(
  ...
  return [...response.cards].sort((left, right) => {
    ...
    const leftAbs = (leftTotal.ars < 0n ? -leftTotal.ars : leftTotal.ars) +
      (leftTotal.usd < 0n ? -leftTotal.usd : leftTotal.usd);
    const rightAbs = (rightTotal.ars < 0n ? -rightTotal.ars : rightTotal.ars) +
