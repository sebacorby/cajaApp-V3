...
import { presentCardPayments } from "./card-payments-presentation.js";
...
export const cardsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/api/card-payments", async (request, reply) => {
    ...
    const normalized = normalizeManualCardAssignments(result);
    return reply.send(await presentCardPayments(normalized));
  });
