...
export class InstallmentProjectionService {
  calculateProjections(
    ...
    for (const row of rows) {
      if (row.rowType !== "transaction" || !row.installmentRaw?.trim()) {
        continue;
      }
    ...
    logger.info(
      ...
      "Installment projections calculated from statement period",
    );
  ...
  parseInstallment(installmentRaw: string): { current: number; total: number } {
    try {
      ...
      // Preserve the historical helper contract for legacy callers. The actual
      // projection path above remains strict and omits invalid rows.
   ...
   * Transaction dates are not an authoritative statement period and must not
   * be used by new code. The acceptance flow will be migrated to pass the
   * persisted statement period explicitly.
   */
  ...
  aggregateByMonth(projections: InstallmentProjection[]): MonthlyProjection[] {
          ...
          logger.warn(
            {
              ...
              amountPesos: projection.amountPesos,
            },
      ...
      if (
        ...
        try {
          ...
          logger.warn(
            {
              rowId: projection.rowId,
              amountDollars: projection.amountDollars,
            },
    ...
    logger.info(
      ...
      "Monthly installment projections aggregated",
    );
