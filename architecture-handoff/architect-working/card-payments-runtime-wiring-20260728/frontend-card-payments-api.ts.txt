...
export interface CardPaymentMovement {
  id: string;
  sourceType: "statement_row" | "manual_purchase" | "reconciliation" | "tax_projection";
  description: string;
  dateIso: string | null;
...
export type IssuerValidationStatus = "matched" | "mismatch" | "ambiguous_mapping";
...
export interface CardPaymentsResponse {
  ...
  issuerValidation: { available: boolean; items: IssuerProjectionValidation[] };
  autoReconciliation?: { adjusted: boolean; adjustments: CardPaymentAdjustmentDetail[] };
...
export async function fetchCardPayments(months = 6, signal?: AbortSignal): Promise<CardPaymentsResponse> {
  const normalized = Math.min(Math.max(Math.trunc(months), 1), 24);
  const response = await fetch(`${API_BASE_URL}/api/card-payments?months=${normalized}`, {
    cache: "no-store",
    signal,
  });
...
export async function fetchCardExchangeRate(signal?: AbortSignal): Promise<CardExchangeRate> {
  const response = await fetch(`${API_BASE_URL}/api/card-statements/exchange-rate`, {
    cache: "no-store",
    signal,
  });
  if (!response.ok) throw await parseError(response);
  return response.json() as Promise<CardExchangeRate>;
}

export async function updateCardExchangeRate(rate: string, effectiveDate: string): Promise<CardExchangeRate> {
  const response = await fetch(`${API_BASE_URL}/api/card-statements/exchange-rate`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ rate, effectiveDate }),
  });
  if (!response.ok) throw await parseError(response);
  return response.json() as Promise<CardExchangeRate>;
}

export async function createManualCardPurchase(input: ManualCardPurchaseInput): Promise<void> {
