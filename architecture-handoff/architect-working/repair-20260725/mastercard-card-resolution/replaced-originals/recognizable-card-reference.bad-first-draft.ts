...
function moneyForRows(rows: readonly FutureDebtRow[]): FutureMoneyBucket {
  let ars = 0n;
  let usd = 0n;
...
function promote(row: FuturePendingRow, label: string): FutureDebtRow | null {
  ...
  const normalized = label.toLocaleLowerCase("es").replace(/[^a-z0-9]+/g, "-");
...
export function resolveRecognizableCardReferences(
  ...
  const normalizedMonths = Array.from(months.values())
    ...
    .map((month) => {
      const cards = month.cards
        .map((card) => ({
          ...card,
          rows: [...card.rows].sort((a, b) => a.id.localeCompare(b.id)),
          totals: moneyForRows(card.rows),
        }))
        .sort((a, b) => a.cardLabel.localeCompare(b.cardLabel, "es"));
      const rows = cards.flatMap((card) => card.rows);
      const warnings = Array.from(new Set(remainingByMonth.get(month.monthKey) ?? []));
      return {
        ...month,
        cards,
  ...
  for (const row of remainingPending) {
    pendingCounts.set(row.diagnostic, (pendingCounts.get(row.diagnostic) ?? 0) + 1);
  }
