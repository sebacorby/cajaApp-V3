      ...
      {data?.baseline ? (
        <div className="rounded-xl border bg-card">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
            <div>
              <p className="font-medium text-foreground">Actualización entre resúmenes</p>
              <p className="mt-0.5 text-xs text-muted-foreground">
                Registrá compras nuevas para mantener los pagos proyectados actualizados hasta la próxima importación.
              </p>
            </div>
            <Button
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
                ...
                <label className="space-y-1">
                  ...
                  <select
                    className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                    value={form.currency}
                    onChange={(event) => setForm((current) => ({ ...current, currency: event.target.value as "ARS" | "USD" }))}
                  >
              ...
              {manualMovements.length > 0 ? (
                <div className="border-t pt-4">
                  <p className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    Compras manuales registradas
                  </p>
                  <div className="space-y-2">
                    {manualMovements.map(({ card, movement }) => {
                      const purchaseId = manualPurchaseId(movement);
                      return (
                        <div key={movement.id} className="flex items-center justify-between gap-3 rounded-lg border bg-muted/15 px-3 py-2">
                          ...
                          >
                            {deletingId === purchaseId ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}
