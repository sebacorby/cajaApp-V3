...
export class InstallmentProjectionService {
  calculateProjections(
    ...
    for (const row of rows) {
      // The normalized field is preferred, but accepted source text is authoritative
      // fallback evidence when the extractor leaves installmentRaw null.
      ...
      try {
        parsed = cardBillingCalendarService.parseInstallment(sequence.raw);
      } catch (error) {
        ...
        logger.warn(
          {
            ...
            resolvedInstallmentRaw: sequence.raw,
