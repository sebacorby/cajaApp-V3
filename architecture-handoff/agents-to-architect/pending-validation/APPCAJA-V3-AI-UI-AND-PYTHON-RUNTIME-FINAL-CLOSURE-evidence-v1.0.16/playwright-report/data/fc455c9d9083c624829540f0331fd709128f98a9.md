...
- complementary:
  ...
  - region "Calidad del dato":
    ...
    - button "Sin clasificar 2 movimientos"
    - button "Alertas activas 1 regla"
    - button "Último dato actualizado 16-jul, 12:25 a. m."
...
- main:
  ...
  - alert: "La afirmación claim-9 contiene valores no presentes en sus fuentes citadas: 100,00%"
  - paragraph: ARS
  - paragraph: N/C
  ...
  - paragraph: Fuentes
  - paragraph: "29"
  - paragraph: 2 movimientos sin clasificar
  ...
  - text: Todavía no hay consultas guardadas.
- contentinfo:
...
> 85  |     await expect(uiResponse).toBeVisible({ timeout: 180_000 });
  ...
  92  |     await page.goto("/");
  93  |     await page.getByRole("button", { name: "Abrir menú" }).click();
  94  |     await page.getByRole("button", { name: "Asesor IA", exact: true }).click();
  95  |     await expect(page.getByTestId("ai-advisor-section")).toBeVisible();
  96  |   } finally {
  97  |     for (const id of [...new Set(interactionIds)]) await request.delete(`${API_BASE_URL}/api/ai-advisor/history/${id}`);
  98  |     for (const id of movementIds) await request.delete(`${API_BASE_URL}/api/movements/manual/${id}`);
  99  |   }
  100 | });
  101 | 
```