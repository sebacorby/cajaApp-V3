...
const envSchema = z.object({
  ...
  OLLAMA_MODEL: z.string().min(1).default("kimi-k2.7-code:cloud"),
  ...
  PYTHON_EXECUTABLE: z.string().trim().default("python"),
...
if (providerTimeout >= parsed.data.AI_JOB_TIMEOUT_MS) {
  console.error(
...
if (majorVersion !== 22) {
  console.error(
    `Node.js 22.x required, found ${nodeVersion}`,
  );
  process.exit(1);
}
