...
const exchangeRateValueSchema = z.string().trim().min(1).refine((value) => {
...
const isoDateSchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "La fecha debe tener formato YYYY-MM-DD").refine((value) => {
