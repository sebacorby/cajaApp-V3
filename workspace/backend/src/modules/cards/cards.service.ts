...
import { CardStatementHistoryConflictError, NotFoundError } from "../../shared/errors.js";
import type {
...
function resolveCardStatementIdentityKey(preview: CardStatementPreview): string {
  const statementNumber = normalizeHistoryPart(preview.source.statementNumber);
  return statementNumber === "unknown" ? "unknown" : `statement:${statementNumber}`;
}

export function resolveCardStatementPeriodKey(preview: CardStatementPreview): string | null {
  const candidates = [
  ...
  for (const candidate of candidates) {
    if (candidate && /^\d{4}-\d{2}-\d{2}$/.test(candidate)) return candidate.slice(0, 7);
  }
  return null;
}
...
function safeParseAmount(value: string | null, currency: "ARS" | "USD"): bigint | null {
  if (!value || !value.trim()) return null;
  try {
    return currency === "ARS" ? parseArgentinePesos(value) : parseDollars(value);
  } catch {
    return null;
  }
}
...
export function calculateCardMoneyEquivalents(
  ...
  const usdEquivalentCents = dollarCents === null
    ? null
    : divideRoundHalfUp(dollarCents * rateCents, 100n);

  const combinedCents = pesoCents === null && usdEquivalentCents === null
    ? null
    : (pesoCents ?? 0n) + (usdEquivalentCents ?? 0n);

  return {
    usdEquivalentPesos: usdEquivalentCents === null
      ? null
      : centsToString(usdEquivalentCents, "ARS"),
    combinedTotalPesos: combinedCents === null
      ? null
      : centsToString(combinedCents, "ARS"),
  };
}
...
export class CardsService {
  ...
  async updateExchangeRate(input: { rate: string; effectiveDate: string }): Promise<CardExchangeRate> {
    const rateCents = parseExchangeRateToCents(input.rate);
    if (rateCents <= 0n) throw new Error("Exchange rate must be greater than zero");
    ...
    logger.info({ pair: USD_ARS_PAIR, effectiveDate: record.effectiveDate }, "Card exchange rate updated");
  ...
  async getPricing(
    ...
    return {
      exchangeRate,
      equivalents: calculateCardMoneyEquivalents(totalPesos, totalDollars, exchangeRate),
      months: months.map((month) => ({
        ...month,
        ...calculateCardMoneyEquivalents(month.totalPesos, month.totalDollars, exchangeRate),
      })),
    };
  }
  ...
  async listStatements(input: {
    ...
    const take = Math.min(Math.max(Math.trunc(input.limit ?? 50) || 50, 1), 100);
    ...
    return records.filter((record) => {
      if (status && status !== "all" && record.status.toLocaleLowerCase("es") !== status) return false;
      ...
      const haystack = [
        ...
        ...record.groups.flatMap((group) => [group.cardLast4, group.holderName]),
  ...
  async getStatementTraceability(statementId: string) {
    ...
    return {
      ...
      aiRun: statement.draft?.aiRun ? {
        ...
        completedAt: statement.draft.aiRun.completedAt?.toISOString() ?? null,
      } : null,
  ...
  async activateStatement(statementId: string) {
    await prisma.$transaction(async (tx) => {
      const target = await tx.cardStatement.findUnique({ where: { id: statementId } });
  ...
  async archiveStatement(statementId: string, reason?: string) {
    await prisma.$transaction(async (tx) => {
      ...
      const replacement = target.isActiveForPeriod && target.historyKey
        ? await tx.cardStatement.findFirst({
            where: {
              historyKey: target.historyKey,
              id: { not: target.id },
      ...
      if (target.isActiveForPeriod && target._count.manualPurchases > 0 && !replacement) {
    ...
    logger.info({ statementId }, "Card statement archived without deleting traceability");
  ...
  async acceptDraft(draftId: string, preview: CardStatementPreview): Promise<AcceptResult> {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const versions = historyKey
        ? await tx.cardStatement.findMany({
            where: { historyKey },
            select: { id: true, status: true, version: true, isActiveForPeriod: true },
            orderBy: { version: "desc" },
          })
        : [];
    ...
    const pricing = await this.getPricing(null, null, result.updatedValues.months);
    return {
      ...result,
  ...
  async getUpdatedValues(fromMonth: string, toMonth: string): Promise<{ exchangeRate: CardExchangeRate; months: MonthlyProjection[] }> {
