...
describe("Galicia Visa deterministic parser", () => {
  ...
  it("normalizes Plan V VAT using the tax amount and preserves the taxable base in source evidence", () => {
    const { preview } = parseGaliciaVisaStatement(raw, 4);
    const planVIva = preview.rows.find((row) => row.originalText.includes("DB IVA $ PLAN V"));

    expect(planVIva?.rowType).toBe("tax");
    expect(planVIva?.editable).toBe(false);
    expect(planVIva?.referenceRaw).toBe("DB IVA $ PLAN V");
    expect(planVIva?.receiptRaw).toBe("040943");
    expect(planVIva?.amountPesos).toBe("13.123,85");
    expect(planVIva?.amountDollars).toBeNull();
    expect(planVIva?.currencyOriginal).toBe("ARS");
    expect(planVIva?.groupId).toBeNull();
    expect(planVIva?.originalText).toContain("62494,55");
    expect(planVIva?.originalText).toContain("13.123,85");
  });
