      ...
      rows: [
        {
          ...
          diagnosticDetail:
            "La ocurrencia no tiene una referencia de tarjeta identificable.",
        },
        {
          id: "projection-generic-visa",
          sourceId: "purchase-generic-visa",
          dateIso: "2026-06-14",
          monthKey: "2026-08",
...
describe("resolveRecognizableCardReferences", () => {
  it("promotes a specific Mastercard Black label without inventing last4", () => {
    ...
    expect(result.months[0].cards[0].rows[0].sourceId).toBe(
      "purchase-mastercard",
    );
    expect(result.months[0].cards[0].rows[0].dateIso).toBe("2026-06-13");
  ...
  it("does not guess a generic VISA reference", () => {
    ...
    expect(result.pendientes.rows[0].sourceId).toBe("purchase-generic-visa");
