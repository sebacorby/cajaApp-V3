...
const OMITTED_ROW_TYPES = new Set([
  "legal_text",
  "section_header",
  "group_header",
]);
...
const groupCount = await page
        ...
        const previewReadyResponse = evidence.importResponses.find(
          (item) =>
            item.status === 200 &&
            typeof item.body === "object" &&
            item.body !== null &&
            (item.body as { status?: string }).status === "preview_ready",
        );
        const preview = (
          previewReadyResponse?.body as
            | { preview?: { rows?: Array<{ rowType: string }> } }
            | undefined
        )?.preview;
        expect(preview, "El cuerpo de la respuesta debe incluir el preview autoritativo.").toBeDefined();
        const previewRows = preview?.rows ?? [];
        const renderableRows = previewRows.filter(
          (row) => !OMITTED_ROW_TYPES.has(row.rowType),
        );
        const rowCount = await page.getByTestId("card-statement-row").count();
        expect(rowCount).toBe(renderableRows.length);




        const transactionCount = previewRows.filter(
          (row) => row.rowType === "transaction",
        ).length;
        const taxCount = previewRows.filter((row) => row.rowType === "tax").length;
        const totalCount = previewRows.filter(
          (row) => row.rowType === "statement_total",
        ).length;
        const groupTotalCount = previewRows.filter(
          (row) => row.rowType === "group_total",
        ).length;
        const futureInstallmentCount = previewRows.filter(
          (row) => row.rowType === "future_installment_reference",
        ).length;
        const chargeCount = previewRows.filter((row) => row.rowType === "charge").length;
        const consolidatedRowCount = previewRows.filter(
          (row) => row.rowType === "consolidated_row",
        ).length;
        const unknownCount = previewRows.filter((row) => row.rowType === "unknown").length;
        const renderedRowClasses = await page
          .locator('[data-testid="card-statement-row"]')
          .evaluateAll((nodes) =>
            nodes.map((node) => (node as HTMLElement).className),
          );
        const toneToCount = (needle: string) =>
          renderedRowClasses.filter((className) => className.includes(needle)).length;
        const renderedTaxOrChargeCount = toneToCount("bg-amber-50");
        const renderedStatementTotalCount = toneToCount("bg-emerald-50");
        const renderedGroupTotalCount = toneToCount("bg-slate-50");
        const renderedFutureInstallmentCount = toneToCount("bg-blue-50");
        const renderedDefaultToneCount = renderedRowClasses.filter(
          (className) =>
        ...
        expect(renderedTaxOrChargeCount).toBe(taxCount + chargeCount);
        expect(renderedStatementTotalCount).toBe(totalCount);
        expect(renderedGroupTotalCount).toBe(groupTotalCount);
        expect(renderedFutureInstallmentCount).toBe(futureInstallmentCount);
        expect(renderedDefaultToneCount).toBe(
          transactionCount + consolidatedRowCount + unknownCount,
        );
