...
test("Recibos de sueldo permite revisar y aceptar un borrador desde Ingresos", async ({ page }) => {
  ...
  const receiptPreview = page.getByTestId("salary-receipt-preview");
  await expect(receiptPreview.locator("input").first()).toHaveValue("Empresa E2E SA");
  await expect.poll(() =>
    receiptPreview.locator("input").evaluateAll((inputs) =>
      inputs.some((input) => (input as HTMLInputElement).value === "Sueldo básico"),
    ),
  ).toBe(true);
