...
test("búsqueda global: teclado, navegación por registro y mobile", async ({ page }) => {
  ...
  try {
    ...
    await expect(
      page.locator('[data-testid^="movement-row-"]').filter({ hasText: description }).first(),
    ).toBeVisible();
    ...
    const desktopDialog = page.getByRole("dialog", { name: "Buscar en CajaApp" });
    await desktopDialog.waitFor({ state: "detached" });
    const desktopOverlay = page.locator('[data-radix-portal], [data-state="open"][data-slot="dialog-overlay"]');
    if (await desktopOverlay.count()) {
      await desktopOverlay.first().waitFor({ state: "detached" });
    }
    await page.getByTestId("header-global-search").click();
    const mobileDialog = page.getByRole("dialog", { name: "Buscar en CajaApp" });
    await expect(mobileDialog).toBeVisible();
    await expect(mobileDialog.getByTestId("global-search-input")).toBeVisible();
    ...
    if (await row.count()) {
      ...
      await expect(
        page.locator('[data-testid^="movement-row-"]').filter({ hasText: description }),
      ).toHaveCount(0);
    }
  }
});