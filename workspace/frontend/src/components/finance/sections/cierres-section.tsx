              ...
              <tbody className="divide-y dark:divide-slate-800">
                {loading ? (
                  ...
                  <tr key={item.id} data-testid={`month-close-row-${item.monthKey}`}>
                    ...
                    <td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === "closed" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300" : "bg-amber-100 text-amber-800 dark:bg-amber-950/30 dark:text-amber-300"}`}>{item.status === "closed" ? "Cerrado" : "Reabierto"}</span></td>
        ...
        <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950" data-testid="month-close-detail">
          {!detail ? (
            ...
            <div className="space-y-5">
              ...
              {detail.status === "closed" ? <p className="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-400"><CheckCircle2 className="h-4 w-4" /> Snapshot activo</p> : <p className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-400"><RotateCcw className="h-4 w-4" /> Snapshot histórico reabierto</p>}
            </div>
          )}
        </aside>
      </div>
