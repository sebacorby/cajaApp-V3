    ...
    <div className="space-y-6" data-testid="incomes-section">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryCard
          icon={Banknote}
          label="Total del mes"
          value={<Amount value={summary?.totalArs ?? "0"} currency="ARS" />}
          detail={
            !isZeroAmount(summary?.totalUsd) ? (
              <><Amount value={summary?.totalUsd ?? "0"} currency="USD" /> adicionales</>
            ) : (
              "ARS y USD se muestran separados"
            )
          }
        ...
        <SummaryCard
          icon={WalletCards}
          label="Recurrentes"
          value={<Amount value={summary?.recurringArs ?? "0"} currency="ARS" />}
          detail={
            !isZeroAmount(summary?.recurringUsd) ? (
              <><Amount value={summary?.recurringUsd ?? "0"} currency="USD" /> · {summary?.recurringSources ?? 0} fuentes</>
            ) : (
              `${summary?.recurringSources ?? 0} fuentes incluidas este mes`
            )
          }
        ...
        <SummaryCard
          ...
          value={<Amount value={summary?.oneOffArs ?? "0"} currency="ARS" />}
          detail={
            !isZeroAmount(summary?.oneOffUsd) ? (
              <><Amount value={summary?.oneOffUsd ?? "0"} currency="USD" /> · {summary?.oneOffCount ?? 0} eventos</>
            ) : (
              `${summary?.oneOffCount ?? 0} eventos este mes`
            )
          }
    ...
    <div className="rounded-2xl border bg-card p-5" data-testid={`income-source-${source.id}`}>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl bg-muted/45 p-3">
          ...
          <p className="mt-1 font-semibold tabular-nums"><Amount value={source.baseAmount} currency={source.currency} /></p>
        </div>
        <div className="rounded-xl bg-muted/45 p-3">
              ...
              <div key={event.id} className="flex items-center justify-between gap-3 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    <Amount value={event.amount} currency={event.currency} /> · {event.status === "actual" ? "real" : "estimado"}
                  </p>
                </div>
                <Button
                  variant="ghost"
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          <p className="font-semibold tabular-nums"><Amount value={month.totalArs} currency="ARS" /></p>
          {!isZeroAmount(month.totalUsd) ? (
            <p className="text-xs tabular-nums text-muted-foreground"><Amount value={month.totalUsd} currency="USD" /></p>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {month.recurring.map((item) => (
          <div key={item.sourceId} className="flex items-center justify-between gap-3 text-sm">
            ...
            <span className="shrink-0 tabular-nums"><Amount value={item.amount} currency={item.currency} /></span>
          </div>
        ))}
        ...
        {month.oneOffs.map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg bg-emerald-50/70 px-2.5 py-2 text-sm dark:bg-emerald-950/15">
            ...
            <div className="flex items-center gap-1">
              <span className="tabular-nums"><Amount value={item.amount} currency={item.currency} /></span>
              <Button
                variant="ghost"
                size="icon"
                className="size-7"
