...
export function SaludFinancieraSection() {
  ...
  if (error || !response) {
    return (
      <Card className="border-rose-200 dark:border-rose-900" data-testid="financial-health-error">
        <CardContent className="flex flex-col items-center gap-3 py-10 text-center">
          <AlertCircle className="size-7 text-rose-600 dark:text-rose-400" aria-hidden="true" />
          <div>
            ...
            {(["ARS", "USD"] as const).map((currency) => {
              ...
              return (
                <div key={currency} className="rounded-xl border p-4">
                  ...
                  <p className="mt-2 text-sm">Meta: <strong><Amount value={goal.targetAmount} currency={currency} /></strong></p>
                  <p className="mt-1 text-sm">Aportado: <strong><Amount value={goal.contributedAmount} currency={currency} /></strong></p>
                  <p className="mt-1 text-sm">Restante: <strong><Amount value={goal.remainingAmount} currency={currency} /></strong></p>
