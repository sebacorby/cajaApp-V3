...
export function AsesorIaSection() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [context, items] = await Promise.all([
        getAiAdvisorContext(range.from, range.to),
        getAiAdvisorHistory(12),
      ]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "No se pudo obtener una respuesta del asesor IA.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (loading) return <div className="grid min-h-72 place-items-center" data-testid="ai-advisor-loading"><div className="text-center text-sm text-muted-foreground"><Loader2 className="mx-auto mb-3 size-6 animate-spin" aria-hidden="true" />Preparando contexto estructurado…</div></div>;


  if (!contextResponse) return <Card className="border-rose-200 dark:border-rose-900"><CardContent className="py-10 text-center"><AlertCircle className="mx-auto size-7 text-rose-600 dark:text-rose-400" /><p className="mt-3 font-medium">No se pudo preparar el asesor IA</p><p className="mt-1 text-sm text-muted-foreground">{error}</p><Button variant="outline" className="mt-4" onClick={() => void load()}>Reintentar</Button></CardContent></Card>;
