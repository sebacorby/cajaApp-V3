      ...
      {enoughHistory ? (
        <>
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
            {first?.label}: {first ? <Amount value={monthBalance(first, currency)} currency={currency} /> : "—"}
            <span aria-hidden="true"> · </span>
            <span className="sr-only">. </span>
            {latest?.label}: {latest ? <Amount value={monthBalance(latest, currency)} currency={currency} /> : "—"}
          </p>
          {!hasActivity ? (
          ...
          <ul className="sr-only" aria-label={`Serie mensual del balance ${currency}`}>
            {months.map((month) => (
              <li key={`${currency}-${month.monthKey}`}>
                {month.label}: <Amount value={monthBalance(month, currency)} currency={currency} />
              </li>
            ))}
          </ul>
        </>
