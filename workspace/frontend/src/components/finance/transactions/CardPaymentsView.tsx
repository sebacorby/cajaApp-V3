...
function TotalToPay({ summary }: { summary: PaymentSummary | null }) {
  if (!summary) {
    return (
      <div className="h-full w-full rounded-xl border bg-card/70 px-5 py-3 shadow-sm">
  ...
  return (
    <div className="flex h-full w-full items-center justify-between gap-6 rounded-xl border bg-card/70 px-5 py-2.5 shadow-sm">
...
export function CardPaymentsView() {
  ...
  useEffect(() => {
    ...
    const normalizeView = () => {
      ...
      if (toolbar instanceof HTMLElement) {
        toolbar.style.minHeight = "3.75rem";
  ...
  const desktopSummary = useMemo(
    () => (
      <div className="pointer-events-none absolute left-0 right-[380px] top-0 z-10 hidden h-[60px] items-stretch lg:flex">
