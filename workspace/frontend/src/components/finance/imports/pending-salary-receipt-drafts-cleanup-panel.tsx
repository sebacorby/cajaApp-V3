...
export function PendingSalaryReceiptDraftsCleanupPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "No se pudieron cargar los borradores pendientes.");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <Card className="shadow-sm" data-testid="pending-salary-receipts-cleanup">
      ...
      <CardContent className="space-y-3">
        <p className="text-sm text-muted-foreground">
          Podés descartar un recibo que todavía no aceptaste ni rechazaste. El PDF quedará disponible para volver a importarlo.
        </p>
        {error ? (
        ...
        {loading ? (
          ...
          drafts.map((draft) => (
            <div key={draft.id} className="flex flex-col gap-3 rounded-lg border p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <p className="font-medium">{draft.document.fileName}</p>
                <p className="mt-1 text-sm text-muted-foreground">Pendiente de validar</p>
              </div>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="destructive" size="sm" disabled={deletingId === draft.id}>
                    {deletingId === draft.id ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Trash2 className="mr-2 size-4" />}
                    Eliminar importación
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>¿Descartar este recibo pendiente?</AlertDialogTitle>
                    <AlertDialogDescription>
                      Se eliminarán el borrador, la extracción y el documento original. Después podrás importar nuevamente el mismo PDF. Esta acción no se puede deshacer.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancelar</AlertDialogCancel>
                    <AlertDialogAction asChild>
                      <Button variant="destructive" onClick={() => void remove(draft)} disabled={deletingId === draft.id}>
