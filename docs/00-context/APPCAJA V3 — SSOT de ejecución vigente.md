...
Estado de código: P1–P4 se conservan como baseline funcional. P5 tiene recibos de sueldo, Centro de importaciones, Conciliación, Cierre mensual y Backup/Restore ya implementados, materializados y aceptados en el workspace canónico. `hideAmounts` está conectado de punta a punta y APP-UX-PRIVACY-002 permanece cerrada / PASS. APP-AGENT-CHAT-001 completó SDD, Phase 1 T001–T005, Foundation T006–T015, US1 T016–T029 y US2 T030–T040. El agente ya dispone de conversaciones/runs/SSE, registry cerrado de 41 tools públicas de lectura, executor con validación Zod/proyección segura/paralelismo sólo `parallelSafe`, persistencia de tool calls/results, Tool Cards, actividad resumida y `ui.navigate` sobre `useFinanceUI`. Próximo bloque técnico autorizado: User Story 3 / R2 T041–T049.
...
Instrucción vigente: APP-AGENT-CHAT-001 está ACTIVADO / EN IMPLEMENTACIÓN bajo `feat/agent-chat`. PRD v1.1.0 + `spec.md` + `plan.md` + `tasks.md` son los artefactos autoritativos del ciclo SDD; T001–T040 están completas. La migración `20260912230000_add_agent_chat` está aplicada/verificada en `dev.db`. APPCAJA-V3-P5-FOCAL-VALIDATION-v1.3.1.md y APP-UX-PRIVACY-002 permanecen históricos / PASS.
Próximo paso obligatorio: ejecutar User Story 3 / R2 T041–T049 en orden TDD, empezando por explicit intent, ambigüedad e idempotencia antes de registrar mutaciones. Ninguna R3/R4 ni import tool se abre todavía; ningún otro vertical funcional se abre mientras APP-AGENT-CHAT-001 permanezca activo.
...
10\. **APP-AGENT-CHAT-001 — ACTIVO / EN IMPLEMENTACIÓN**: nuevo Agente IA conversacional global, separado de `Asesor IA`. Rama vigente: `feat/agent-chat`. PRD v1.1.0 y ciclo SDD completo (`spec.md`, `plan.md`, `tasks.md` + design artifacts). Phase 1 T001–T005, Foundation T006–T015, User Story 1 T016–T029 y User Story 2 T030–T040 completadas. Además del MVP conversacional, el agente ya consulta services reales mediante un registry cerrado de 41 tools públicas de lectura, valida argumentos/proyecta resultados de forma segura, persiste tool calls, muestra Tool Cards/actividad y puede ejecutar `ui.navigate` manteniendo el panel abierto. La migración Agent* `20260912230000_add_agent_chat` está aplicada y verificada en `dev.db`. Siguiente checkpoint autorizado: T041–T049 — User Story 3 / R2 con explicit intent e idempotencia. La arquitectura mantiene tools explícitas sobre services existentes y prohíbe acceso directo del modelo a Prisma/SQL/filesystem/shell.
