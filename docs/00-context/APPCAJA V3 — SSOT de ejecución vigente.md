...
Fase activa: P5 — INGESTA FINANCIERA REAL. APP-P5-MATERIALIZATION-001 y APP-P5-FOCAL-VALIDATION-001 quedaron CERRADOS / PASS con la campaña v1.3.1 (evidencia en accepted). APP-UX-PRIVACY-002 — conectar hideAmounts de punta a punta (backend, contrato frontend, toggle en Settings, componente `Amount` de enmascarado transversal y pruebas) — quedó CERRADO / PASS el 10/09/2026 tras la campaña de cierre completa (suite backend 176/176, frontend typecheck/lint/build limpio, smoke API, Playwright completo real, `dev.db` restaurado a hash exacto). No queda bloque activo. Deuda separada y no bloqueante: el subconjunto de specs que invoca IA real en vivo (Asesor IA, importación real con extracción por IA) está BLOCKED por falta de suscripción/créditos en la cuenta de Ollama para el modelo cloud configurado — ver Registro de decisiones y sección 6. APP-RECONCILIATION-001 permanece implementado y congelado.  
Backlog funcional original: COMPLETO EN DRIVE.  
Backlog original y paridad útil P1–P4: BASELINE FUNCIONAL IMPLEMENTADO. La auditoría post-prototipo v1.2.0 confirma que la aplicación supera al prototipo y reabre únicamente la regresión APP-UX-PRIVACY-002, ahora bloque activo; las demás brechas detectadas son mejoras P1/P2 no bloqueantes.  
...
Estado de código: P1–P4 se conservan como baseline funcional. P5 tiene recibos de sueldo, Centro de importaciones, Conciliación, Cierre mensual y Backup/Restore ya implementados, materializados y aceptados en el workspace canónico. La auditoría del prototipo detectó además una regresión de privacidad: hideAmounts permanece en Prisma, pero no está conectado de punta a punta en Settings, preferencias y UI — es el bloque activo vigente.  
...
Instrucción vigente: ninguna instrucción de campaña activa; APPCAJA-V3-P5-FOCAL-VALIDATION-v1.3.1.md queda COMPLETADA / PASS y pasa a histórico. APP-UX-PRIVACY-002 cierra PASS el 10/09/2026 y pasa a histórico también.  
Próximo paso obligatorio: no hay bloque activo pendiente de campaña. Corresponde al arquitecto/usuario decidir el siguiente vertical: activar mejoras P1/P2 en cola (sección 9, ítem 8), o resolver primero la deuda de suscripción/créditos de Ollama para poder validar en vivo el Asesor IA y las importaciones reales con IA (sección 6).
...
P1–P4 quedan cerradas como baseline funcional; la regresión puntual APP-UX-PRIVACY-002 quedó cerrada en PASS el 10/09/2026. P5 está activa. APP-SALARY-RECEIPT-001, APP-IMPORT-CENTER-001, APP-MONTH-CLOSE-001, APP-BACKUP-RESTORE-001 y APP-P5-FOCAL-VALIDATION-001 están cerrados en PASS. APP-RECONCILIATION-001 está implementado.
...
8\. Mejoras P1/P2 y deuda técnica — EN COLA, no bloqueantes. No hay bloque activo; requiere nueva decisión del arquitecto/usuario para activar el siguiente ítem.  
...
Regla: un bloque activo por vez; no queda ninguno abierto tras el cierre de APP-UX-PRIVACY-002.
