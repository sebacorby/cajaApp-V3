...
Instrucción vigente: APP-AGENT-CHAT-001 está ACTIVADO en fase pre-implementación bajo `feat/agent-chat`; el PRD v1.1.0 es la autoridad de producto y debe convertirse a Spec Kit sin reabrir decisiones ya cerradas. APPCAJA-V3-P5-FOCAL-VALIDATION-v1.3.1.md y APP-UX-PRIVACY-002 permanecen históricos / PASS.  
Próximo paso obligatorio: transformar `specs/001-chat-con-ia/PRD.md` v1.1.0 en los artefactos Spec Kit (`spec.md`, luego `plan.md` y `tasks.md`) y recién después iniciar implementación sobre `feat/agent-chat`. Ningún otro vertical funcional se abre mientras APP-AGENT-CHAT-001 permanezca activo.
...
10\. **APP-AGENT-CHAT-001 — ACTIVO / PRE-IMPLEMENTACIÓN**: nuevo Agente IA conversacional global, separado de `Asesor IA`. Rama: `feat/agent-chat`. PRD autoritativo: `specs/001-chat-con-ia/PRD.md` v1.1.0. Próxima secuencia obligatoria: `spec.md` → `plan.md` → `tasks.md` → implementación incremental → gates. Entrada visual principal: launcher circular flotante persistente abajo a la derecha; panel desktop / full-screen mobile. Tools sobre services reales de CajaApp, memoria local, streaming, approvals R3/R4, sin acceso directo a Prisma/SQL/filesystem/shell.  
