...
Estado de código: P1–P4 se conservan como baseline funcional. P5 tiene recibos de sueldo, Centro de importaciones, Conciliación, Cierre mensual y Backup/Restore ya implementados, materializados y aceptados en el workspace canónico. `hideAmounts` está conectado de punta a punta y APP-UX-PRIVACY-002 permanece cerrada / PASS. APP-AGENT-CHAT-001 completó el ciclo SDD (`spec.md`, `plan.md`, `research.md`, `data-model.md`, contratos, `quickstart.md`, `tasks.md`) y la Phase 1 Setup T001–T005: prompt contractual, configuración `AGENT_*`, contrato `AgentChatProvider`, tipos transversales y schemas Zod base. Próximo bloque técnico: Phase 2 Foundation T006–T015.  
...
Instrucción vigente: APP-AGENT-CHAT-001 está ACTIVADO / EN IMPLEMENTACIÓN bajo `feat/agent-chat`. PRD v1.1.0 + `spec.md` + `plan.md` + `tasks.md` son los artefactos autoritativos del ciclo SDD; T001–T005 están completas. APPCAJA-V3-P5-FOCAL-VALIDATION-v1.3.1.md y APP-UX-PRIVACY-002 permanecen históricos / PASS.  
Próximo paso obligatorio: ejecutar Phase 2 Foundation T006–T015 en orden TDD: tests de persistencia/provider/eventos antes de schema/migración/adapters, validar migración sobre copia de `dev.db`, y detenerse en el checkpoint Foundation antes de iniciar US1. Ningún otro vertical funcional se abre mientras APP-AGENT-CHAT-001 permanezca activo.
...
10\. **APP-AGENT-CHAT-001 — ACTIVO / EN IMPLEMENTACIÓN**: nuevo Agente IA conversacional global, separado de `Asesor IA`. Rama: `feat/agent-chat`. PRD v1.1.0 y ciclo SDD completo (`spec.md`, `plan.md`, `tasks.md` + design artifacts). Phase 1 T001–T005 completada; siguiente checkpoint Phase 2 Foundation T006–T015. Entrada visual principal: launcher circular flotante persistente abajo a la derecha; panel desktop / full-screen mobile. Tools sobre services reales de CajaApp, memoria local, streaming, approvals R3/R4, sin acceso directo a Prisma/SQL/filesystem/shell.  
