...
Estado de código: P1–P4 se conservan como baseline funcional. P5 tiene recibos de sueldo, Centro de importaciones, Conciliación, Cierre mensual y Backup/Restore ya implementados, materializados y aceptados en el workspace canónico. `hideAmounts` está conectado de punta a punta y APP-UX-PRIVACY-002 permanece cerrada / PASS. APP-AGENT-CHAT-001 completó SDD, Phase 1 T001–T005, Foundation T006–T015, US1 T016–T029, US2 T030–T040 y US3 T041–T049. El registry cerrado expone 60 tools públicas: las 41 R0/R1 previas más 19 R2 exactas del contrato. R2 usa explicit intent derivado localmente del texto actual, bloquea referencias targeteadas ambiguas, ejecuta writes serialmente y aplica idempotencia durable `runId:providerCallId`, persistiendo éxito antes de devolver el tool result. Tool Cards distinguen acciones R2 ejecutadas/no ejecutadas. Próximo bloque técnico autorizado: User Story 4 / approvals R3-R4 T050–T059.
...
Instrucción vigente: APP-AGENT-CHAT-001 está ACTIVADO / EN IMPLEMENTACIÓN bajo `feat/agent-chat`. PRD v1.1.0 + `spec.md` + `plan.md` + `tasks.md` son los artefactos autoritativos del ciclo SDD; T001–T049 están completas. La migración `20260912230000_add_agent_chat` está aplicada/verificada en `dev.db`. APPCAJA-V3-P5-FOCAL-VALIDATION-v1.3.1.md y APP-UX-PRIVACY-002 permanecen históricos / PASS.
Próximo paso obligatorio: ejecutar User Story 4 / approvals R3-R4 T050–T059 en orden TDD, empezando por approval requerida, hash de argumentos y prueba de que R3/R4 nunca ejecutan sin approval válida. Ninguna import tool T060+ se abre todavía; ningún otro vertical funcional se abre mientras APP-AGENT-CHAT-001 permanezca activo.
...
10\. **APP-AGENT-CHAT-001 — ACTIVO / EN IMPLEMENTACIÓN**: nuevo Agente IA conversacional global, separado de `Asesor IA`. Rama vigente: `feat/agent-chat`. PRD v1.1.0 y ciclo SDD completo (`spec.md`, `plan.md`, `tasks.md` + design artifacts). Phase 1 T001–T005, Foundation T006–T015, User Story 1 T016–T029, User Story 2 T030–T040 y User Story 3 T041–T049 completadas. El agente dispone de 60 tools públicas cerradas: 41 R0/R1 y 19 R2. Las R2 explícitas delegan a services reales sin confirmación redundante, las referencias ambiguas no autorizan writes, las writes son seriales y la idempotencia durable usa `runId:providerCallId`; Tool Cards muestran resultado real. La migración Agent* `20260912230000_add_agent_chat` está aplicada y verificada en `dev.db`. Siguiente checkpoint autorizado: T050–T059 — User Story 4 / approvals R3-R4. La arquitectura mantiene tools explícitas sobre services existentes y prohíbe acceso directo del modelo a Prisma/SQL/filesystem/shell.
